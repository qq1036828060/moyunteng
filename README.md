...
- [x] 支付扫码接口增加按 `主机ID + 实例位` 的数据库原子锁，并发请求不再覆盖正在执行的二维码和坐标操作。
- [x] 实例位锁在任务完成或异常时按持有者释放，并设置 120 秒自动过期保护。
- [x] 新增 `POST /cloud/saohao/pay/bank` 银行卡支付命令，自动刷新待付款订单并固定选择第一张已绑定银行卡。
- [x] 新增 `POST /cloud/saohao/pay/bankcode` 短信验证码接口，根据第一阶段 `taskId` 接管实例位锁、输入验证码并确认支付。
- [x] 银行卡支付使用 UI 层级文本定位关键按钮，保留 `720 x 1280` 参考坐标作为兼容兜底；等待验证码阶段独占实例位最长 600 秒。
